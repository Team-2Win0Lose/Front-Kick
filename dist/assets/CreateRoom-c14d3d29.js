...
`,_n=o.div`
  ...
  cursor: pointer;
  font-size: 24px;
`,Ce=()=>s(Pn,{children:s(In,{children:s(yn,{})})}),Pn=o.div`
  height:100%;
  overflow-x: hidden;
`,In=o.div`
  height:100%;
  background-color: #1F1F45;
  overflow-x: hidden;
`,{kakao:_}=window,zn=a=>{let e=[];const t=I(),{detailMeetingPlace:n,meetingPlace:r}=E(i=>i.summary);return R.useEffect(()=>{const i=document.getElementById("map"),c={center:new _.maps.LatLng(37.566826,126.9786567),level:3},d=new _.maps.Map(i,c),g=new _.maps.services.Places,p=new _.maps.InfoWindow({zIndex:1});m();function m(){let h=a.searchKeyword;if(!h.replace(/^\s+|\s+$/g,""))return console.log("키워드를 입력해주세요!"),!1;g.keywordSearch(h,b)}function b(h,l,D){if(l===_.maps.services.Status.OK)S(h),k(D);else if(l===_.maps.services.Status.ZERO_RESULT){alert("검색 결과가 존재하지 않습니다.");return}else if(l===_.maps.services.Status.ERROR){alert("검색 결과 중 오류가 발생했습니다.");return}}function S(h){const l=document.getElementById("places-list"),D=document.getElementById("search-result"),M=document.createDocumentFragment(),T=new _.maps.LatLngBounds;l&&f(l),w();for(var P=0;P<h.length;P++){let V=new _.maps.LatLng(h[P].y,h[P].x),O=v(V,P),te=x(P,h[P]);T.extend(V),function(U,de){_.maps.event.addListener(U,"mouseover",function(){C(U,de)}),_.maps.event.addListener(U,"mouseout",function(){p.close()}),te.onmouseover=function(){C(U,de)},te.onmouseout=function(){p.close()}}(O,h[P].place_name),M.appendChild(te)}l&&l.appendChild(M),D&&(D.scrollTop=0),d.setBounds(T)}function x(h,l){const D=document.createElement("li");let M=`
    ...
    `;D.innerHTML=M,D.className="item",D.onclick=function(){var P;(P=a.handleSearchLocation)==null||P.call(a,l.place_name,l.address_name)};const T=D.querySelector(".checkbox-input");return T==null||T.addEventListener("click",()=>{t(Ne({meetingPlace:l.place_name,detailMeetingPlace:n,meetingPlaceAddress:l.address_name})),D.classList.toggle("checked")}),D}function v(h,l,D){var M="https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/marker_number_blue.png",T=new _.maps.Size(36,37),P={spriteSize:new _.maps.Size(36,691),spriteOrigin:new _.maps.Point(0,l*46+10),offset:new _.maps.Point(13,37)},V=new _.maps.MarkerImage(M,T,P),O=new _.maps.Marker({position:h,image:V});return O.setMap(d),e.push(O),O}function w(){for(var h=0;h<e.length;h++)e[h].setMap(null);e=[]}function k(h){const l=document.getElementById("pagination");let D=document.createDocumentFragment(),M;for(;l.hasChildNodes();)l.lastChild&&l.removeChild(l.lastChild);for(M=1;M<=h.last;M++){const T=document.createElement("div");T.href="#",T.innerHTML=M.toString(),M===h.current?T.className="on":T.onclick=function(P){return function(){h.gotoPage(P)}}(M),D.appendChild(T)}l.appendChild(D)}function C(h,l){const D='<div style="padding:10px; z-index:1;" class="marker-title">'+l+"</div>";p.setContent(D),p.open(d,h)}function f(h){for(;h.hasChildNodes();)h.lastChild&&h.removeChild(h.lastChild)}},[a.searchKeyword,n]),s(Nn,{children:y(jn,{children:[s(Fn,{id:"map",className:"map"}),y(Hn,{id:"search-result",children:[s(Bn,{children:y(On,{children:[a.searchKeyword," 관련 검색결과"]})}),s(An,{children:s(Wn,{id:"places-list"})}),s(Ln,{id:"pagination"})]})]})})},Nn=o.div`
  align-items: center;
  justify-content: center;
`,jn=o.div`
  display: flex;
...
`,wi=o.div`
  ...
  align-items: center;
  margin-bottom: 20px;
`,se=[{idx:1,name:"여행 위주",origName:"# 여행 위주"},{idx:2,name:"자유로운",origName:"# 자유로운"},{idx:3,name:"조용히",origName:"# 조용히"},{idx:4,name:"숙박",origName:"# 숙박"},{idx:5,name:"당일치기",origName:"# 당일치기"},{idx:6,name:"시끌벅적",origName:"# 시끌벅적"},{idx:7,name:"우천시 취소",origName:"# 우천시 취소"},{idx:8,name:"시간 조율",origName:"# 시간 조율"},{idx:9,name:"남성",origName:"# 남성"},{idx:10,name:"여성",origName:"# 여성"},{idx:11,name:"10대",origName:"# 10대"},{idx:12,name:"20대",origName:"# 20대"},{idx:13,name:"30대",origName:"# 30대"},{idx:14,name:"40대",origName:"# 40대"},{idx:15,name:"50대",origName:"# 50대"},{idx:16,name:"60대",origName:"# 60대"},{idx:17,name:"70대",origName:"# 70대"},{idx:18,name:"80대",origName:"# 80대"},{idx:19,name:"90대",origName:"# 90대"}],Si=()=>{const[a,e]=R.useState(!0),[t,n]=R.useState([...se]);let[r,i]=R.useState([]);const[c,d]=R.useState(),g=I(),{img:p,content:m,title:b,tag:S}=E(f=>f.summary),x=()=>{n([...se]),e(f=>!f)},v=(f,h,l)=>f.split(h).join(l),w=f=>{const h=f.target.value,D=se.map(M=>{const T={...M};return T.name.indexOf(h)>-1&&(T.name=v(T.name,h,`<b style='color:black'>${h}</b>`)),T}).filter(M=>M.name.indexOf(h)>-1);n([...D])},k=f=>()=>{if(x(),!r.some(l=>l.idx===f.idx)){const l=new Set([...r,f]),D=[...r,f];i([...Array.from(l)]),g(G({tag:D.map(M=>M),img:p,content:m,title:b}))}},C=f=>{const h=r.filter(l=>l.idx!==f);i([...h]),g(G({tag:h.map(l=>l),img:p,content:m,title:b}))};return S.length!==0&&(r=[...S]),y(Ci,{children:[s(ki,{onClick:x,children:y(Ri,{children:[s(Ei,{children:"태그를 선택해주세요."}),s(Di,{filterState:a})]})}),a?s(_i,{children:r==null?void 0:r.map(f=>s(zi,{hover:f.idx===c,onMouseOver:()=>{d(f.idx)},onMouseOut:()=>{d(void 0)},onClick:()=>{C(f.idx)},children:f.origName},f.idx))}):y(Ii,{children:[s(Pi,{onChange:w,placeholder:"태그 검색하기"}),s(Mi,{}),s(Ti,{children:t==null?void 0:t.map(f=>s(Ni,{onClick:k(f),dangerouslySetInnerHTML:{__html:f.name}},f.idx))})]})]})},Ci=o.div``,ki=o.div`
  padding: 10px;
  border: 1px solid #ccc;
  width: 300px;
  border-radius: 10px;
...
`,is=["🏠 숙박","🍚 음식점","🏖️ 관광지"],ss=()=>s(as,{children:is.map((a,e)=>s(Xi,{title:a,index:e},e))}),as=o.div`
  ...
  justify-content: center;
  overflow: hidden;
`,os=()=>{const[a,e]=R.useState(!1),t=Ve(),n=I(),{post:r,loading:i}=E(l=>l.accompany),c=["경기 일정 선택","모임장소","일정 카드 선택","상세 내용","상세 내용","내 동행 일정"],d=E(l=>l.selecteditem.attraction),g=E(l=>l.selecteditem.food),m={accommodation:E(l=>l.selecteditem.house).map(l=>({contentid:l.contentid,title:l.title,firstimage:l.firstimage})),restaurant:g.map(l=>({contentid:l.contentid,title:l.title,firstimage:l.firstimage})),attraction:d.map(l=>({contentid:l.contentid,title:l.title,firstimage:l.firstimage}))},[b,S]=R.useState(0),x=()=>{b>0&&S(l=>l-1)},w=E(l=>l.summary.tag).map(l=>l.name).join(","),k={token:E(l=>l.auth.token),schedule_id:E(l=>l.summary.game_schedule_id),home_team_name:E(l=>l.summary.homename),team_id:E(l=>l.summary.team_id),thumbnail:"imgDummy",title:E(l=>l.summary.title),content:E(l=>l.summary.content),tag_list:w,tour_card_id_list:m,meeting_place:E(l=>l.summary.meetingPlace),meeting_place_address:E(l=>l.summary.meetingPlaceAddress),detail_meeting_place:E(l=>l.summary.detailMeetingPlace),term:E(l=>l.summary.date),min_num:E(l=>l.summary.minNum),max_num:E(l=>l.summary.maxNum),now_status:0};R.useEffect(()=>{n(Tt(k))},[a]);const C=async()=>{b<c.length-1?S(l=>l+1):b===c.length-1&&(e(!0),console.log(r,i),setTimeout(()=>{n(_t()),n(Pt()),t("/")},3e3))},f=b===5?"완료":"다음";let h=null;switch(b){case 0:h=Ce;break;case 1:h=$n;break;case 2:h=ss;break;case 3:h=di;break;case 4:h=ji;break;case 5:h=Bi;break;default:h=Ce}return y(ds,{children:[y(ls,{children:[s(cs,{onClick:x}),s(rs,{children:c[b]}),s(gs,{onClick:()=>t("/")})]}),s(hs,{children:s(h,{})}),s(ps,{children:s(fs,{onClick:C,children:f})})]})},rs=o.h1`
  display: flex;
  color: #000;
  font-size: 30px;
  font-weight: bold;
